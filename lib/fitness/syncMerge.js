...
// workout library/schedule/log/gyms, and the Apple Shortcut posting Health
// data straight to /api/fitness/health-sync) and both updates must survive a
// merge, not clobber each other.
//
// State shape: { workouts: { updatedAt, list }, gyms: { updatedAt, list },
// schedule: { updatedAt, days }, logs: { [dateISO]: { manual, health } } }.
//
// `workouts` (the named-workout library), `gyms` (saved gym profiles + their
// equipment inventories) and `schedule` (which workout you're planning for
// each of the next few weeks) are each a single last-write-wins blob -- all
// three are edited rarely, from one place at a time, so there's no need to
// diff/merge them field-by-field.
//
...
export function normalizeFitnessState(state) {
  ...
  return { workouts, gyms, schedule, logs };
}

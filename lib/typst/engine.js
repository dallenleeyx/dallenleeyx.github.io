...
// Typst editor. Served same-origin as plain static files under public/
// (copied from node_modules at build/dev time by
// scripts/copy-typst-wasm.js) rather than committed into this repo
// (would balloon it by ~29MB with no way to shrink back down), pulled
// from a third-party CDN (one more thing that has to stay up), or -- as
// this used to do -- streamed through a Route Handler: Vercel's
// Serverless Function responses are capped at 4.5MB, which silently
// truncated the 28MB compiler and surfaced client-side as "WebAssembly
// compilation aborted: Network error: Response body loading was
// aborted". Static files under public/ are served off Vercel's CDN

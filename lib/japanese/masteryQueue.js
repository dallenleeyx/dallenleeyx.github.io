...
export function gradeMasteryQueue(state, isCorrect) {
  ...
  return {
    next: {
      // sessionItems must track each item's latest attempts/correctAttempts
      // (not just the initial zeroed copies from buildMasteryQueue), since
      // isLessonComplete reads this array -- a lesson-complete check right
      // after this grade needs to see this item's just-updated counts.
      sessionItems: sessionItems.map((i) => (i === current ? item : i)),

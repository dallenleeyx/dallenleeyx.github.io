...
export const SYNC_SECTIONS = ['vocab', 'writing', 'furigana', 'vocabMastery', 'grammarMastery', 'srs', 'planBundle', 'lessonNotes'];

const EMPTY_PLAN_BUNDLE = { plan: {}, planDone: {}, planDaily: {}, planEdits: { done: {}, daily: {} } };

export function normalizeSyncState(state) {
  const empty = { vocab: {}, writing: {}, furigana: {}, vocabMastery: {}, grammarMastery: {}, srs: {}, planBundle: EMPTY_PLAN_BUNDLE, lessonNotes: {} };
...
// Journal entries, keyed by id. Per entry, whichever copy was touched more
// recently wins outright -- including a "deleted: true" tombstone, which is
// just another edit as far as this is concerned. That's what keeps a delete
// made on one device from being resurrected by a stale copy another device
// still holds and syncs later: the merge compares updatedAt, not presence.
// Entries that only exist on one side (new since the last sync) pass through
// untouched, so two devices adding different lessons both survive.
export function mergeLessonNotes(localMap, remoteMap) {
  const out = { ...(localMap || {}) };
  Object.keys(remoteMap || {}).forEach((id) => {
    const r = remoteMap[id];
    const l = out[id];
    if (!l) { out[id] = r; return; }
    if ((Number(r.updatedAt) || 0) > (Number(l.updatedAt) || 0)) out[id] = r;
  });
  return out;
}
...
export function mergeSyncState(localState, remoteState) {
  ...
  return {
    ...
    lessonNotes: mergeLessonNotes(local.lessonNotes, remote.lessonNotes),
    planBundle: {

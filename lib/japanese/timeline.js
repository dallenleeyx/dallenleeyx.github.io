...
// are plain English rather than routed through the Japanese-only i18n
// dictionary, since this module (and the combined Dashboard that consumes
// getDueToday) is language-agnostic chrome shared with the math side.
...
// Dashboard "due today" contract shared with the math side's
// getDueMathTasks: a plain array of { id, kind, label, level, lesson,
// count }. `japaneseState` is { plan, planDone, planDaily, vocabQueue,
// grammarQueue } -- the caller builds vocabQueue/grammarQueue once from the
// data files via buildLessonQueue and passes them in, since this module
// doesn't import the data files itself.
export function getDueToday(japaneseState, now = Date.now()) {
  const { plan, planDone, planDaily, vocabQueue, grammarQueue } = japaneseState;
  const today = todayISO(new Date(now));
  const phase = planPhase(plan, today);

  if (phase === 'learn') {
    const vLeft = vocabQueue.length - doneCount(vocabQueue, planDone);
    const gLeft = grammarQueue.length - doneCount(grammarQueue, planDone);
    const vToday = doneTodayCount(vocabQueue, planDone, today);
    const gToday = doneTodayCount(grammarQueue, planDone, today);
    const vQuota = Math.max(0, plan.vocabPerDay - vToday);
    const gQuota = Math.max(0, plan.grammarPerDay - gToday);
    const items = [];
    if (vLeft > 0 && vQuota > 0) {
      nextUndone(vocabQueue, planDone, vQuota).forEach((task) => {
        items.push({ id: taskKey(task), kind: 'vocab', label: `Vocab ${task.level} Lesson ${task.lesson}`, level: task.level, lesson: task.lesson, count: 1 });
      });
    }
    if (gLeft > 0 && gQuota > 0) {
      nextUndone(grammarQueue, planDone, gQuota).forEach((task) => {
        items.push({ id: taskKey(task), kind: 'grammar', label: `Grammar ${task.level} Lesson ${task.lesson}`, level: task.level, lesson: task.lesson, count: 1 });
      });
    }
    return items;
  }
  ...
  if (phase === 'review') {
    const day = (planDaily && planDaily[today]) || {};
    return REVIEW_HABITS.filter((h) => !day[h.id]).map((h) => ({
      id: `review:${h.id}`, kind: 'review', label: h.label, level: null, lesson: null, count: 1,
    }));
  }

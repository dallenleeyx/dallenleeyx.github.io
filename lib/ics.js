...
// Matches an event title against each course's explicit `code` field (the
// one meant to mirror the school's own module code) and, failing that,
// falls back to glyph/nickname for courses that never had a code set --
// preferring the longest matching code so e.g. "MA2101" wins over a shorter
// false-positive substring.
export function matchCourseForTitle(title, courses) {
  const upper = title.toUpperCase();
  let best = null;
  for (const c of courses) {
    const codes = [c.code, c.glyph, c.nickname].filter(Boolean);

  ...
  return (
    <html lang="en" data-theme="light" data-scroll-behavior="smooth">
      <head>
        ...
        {/* Every section's fonts loaded upfront as real <link> tags (not a CSS
            @import, which chains a fetch off the stylesheet instead of
            starting immediately with the HTML), so switching between
            /japanese, /fitness, /math, /bible, and /jobs never waits on a
            fresh font fetch. Math uses STIX Two Text (headings/body) + DM
            Mono (code). Bible uses Cormorant Garamond for headings (a
            classic literary display serif) and Lora for body/verse text (a
            warm, highly readable book serif) -- deliberately distinct from
            Math's typesetting-oriented STIX Two Text, since long-form
            scripture reading wants a different feel than math notation.
            Jobs uses Inter, a plain modern UI sans that fits a practical
            tracker better than any of the other sections' more expressive
            typefaces. */}

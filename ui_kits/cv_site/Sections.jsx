...
function BlogSection({ posts, onAdd, onOpen, onDelete, owner }) {
  return (
    <section id="blog" className="visible">
      <SectionLabel action={owner ? <AddBtn onClick={onAdd} label="Write Post"/> : null}>Blog</SectionLabel>
      {posts.length === 0 ? (
        <div className="empty-state">
          <div className="empty-icon">✍️</div>
          <div className="empty-title">No posts yet</div>
          <div className="empty-sub">{owner ? <>Click <strong>Write Post</strong> to publish — full LaTeX support.</> : <>Posts will appear here.</>}</div>
        </div>
      ) : (
          ...
          {posts.map(p => (
            <a key={p.slug} className="blog-post" href="#" onClick={(e)=>{e.preventDefault();onOpen(p.slug);}}>
              <div className="blog-post-meta">
                ...
                {owner && p.userPost ? (
                  <button className="icon-btn" style={{marginLeft:'auto'}} onClick={(e)=>{e.preventDefault();e.stopPropagation();onDelete(p.slug);}} title="Delete post">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/></svg>
                  </button>
                ) : null}
              </div>
  ...
  return (
    <section id="blog" className="visible">
      ...
      <article>
        ...
        <div className="post-body" dangerouslySetInnerHTML={{__html: window.renderMarkdown(post.body || '')}}/>
      </article>
    </section>
  );
}
